...
open class BaseApp :
  ...
  internal interface ApplicationEntrypoint {
    ...
    fun mascotManager(): MascotManager
  }
  ...
  private val mascotManager by lazy {
    EarlyEntryPoints.get(this, ApplicationEntrypoint::class.java).mascotManager()
  }
  ...
  override fun onCreate() {
    ...
    if (preferences.markerColor.isBlank()) {
      preferences.markerColor = String.format("#%06X", (java.util.Random().nextInt(0xFFFFFF + 1)))
    }

    if (preferences.face.isBlank()) {
      preferences.face = mascotManager.getRandomMascotId()
    }
