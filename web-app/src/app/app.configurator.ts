...
export class AppConfigurator {
  ...
  setTemaUsuario() {
    var tema: any = {
      color: sessionStorage.getItem('color') ?? 'gray',
      dark: sessionStorage.getItem('dark'),
      toggle: sessionStorage.getItem('toggle'),
    }
    if (tema?.color) {
      this.layoutService.layoutConfig.update((state) => ({ ...state, primary: tema?.color }));
      this.applyTheme('primary', tema?.color);
    }
    if (tema?.dark == 'true')
      this.layoutService.layoutConfig.update((state) => ({ ...state, darkTheme: !state.darkTheme }));
    if (tema?.toggle == 'false')
        ...
        return {
          semantic: {
            ...
            colorScheme: {
              light: {
                primary: {
                  color: (this.layoutService.getPrimary() == 'gray' ? '{primary.700}' : '{primary.500}'),
                  contrastColor: '#ffffff',
